...
export function registerAnalyzeCommands(
  ...
  return [
    vscode.commands.registerCommand("archexa.analyze", async () => {
      ...
      const panelTitle = "Analyze";

      const panel = ArchexaWebviewPanel.getOrCreate(
        "analyze", panelTitle, vscode.ViewColumn.Beside, extensionUri
      );
      panel.reset(panelTitle, "analyze");
      panel.setMeta(
        cfg.get<string>("model") ?? "gpt-4o",
        vscode.workspace.workspaceFolders?.[0]?.name ?? ""
      );

      statusBar.setRunning("Analyze", tokenSource);
      sidebar.showProgress("Starting analysis...", 0);
